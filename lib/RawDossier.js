...
function RawDossier(endpoint, seed) {
    ...
    this.addFolder = (fsFolderPath, barPath, depth, callback) => {
        ...
        if (depth === 0) {
            ...
            loadBarForPath(barPath, (err, dossierContext) => {
                ...
                dossierContext.archive.addFolder(fsFolderPath, dossierContext.relativePath, isEncrypted, callback);
            });
        }
    };

    this.addFile = (fsFilePath, barPath, depth, callback) => {
        if (typeof depth === "function") {
            callback = depth;
            depth = undefined;
        }
        if (depth === 0) {
            bar.addFolder(fsFilePath, barPath, (err, barMapDigest) => callback(err, barMapDigest));
        } else {
            loadBarForPath(barPath, (err, dossierContext) => {
                if (err) {
                    return callback(err);
                ...
                dossierContext.archive.addFile(fsFilePath, dossierContext.relativePath, isEncrypted, callback);
            });
        }
    };
