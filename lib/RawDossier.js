...
function RawDossier(endpoint, seed) {
    ...
    this.loadBarForPath = (path, callback) => {
        ...
        function __loadBarForPathRecursively(rawDossier, prefixPath, relativePath, callback) {
            rawDossier.listFiles((err, files) => {
                ...
                if (barPath) {
                    return callback(undefined, {rawDossier, prefixPath, relativePath});
                } else {
                            ...
                            for (let mount of manifest.mounts) {
                                ...
                                if (mount.localPath === localPath && mount.mountName === name) {
                                    ...
                                    if (pathRest.length === 0) {
                                        return callback(undefined, {rawDossier: internalRawDossier, prefixPath: splitPath.join("/"),relativePath: "/"})
                                    } else {
                                        return __loadBarForPathRecursively(internalRawDossier, splitPath.join("/"), pathRest.join("/"), callback);
                                    }
                                }
                            }
