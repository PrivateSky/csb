function RawDossier(endpoint, seed, cache) {
    ...
    this.rename = (srcPath, dstPath, options, callback) => {
        const defaultOpts = {ignoreMounts: true};
        if (typeof options === 'function') {
            callback = options;
            options = {};
        }
        ...
        this.getArchiveForPath(srcPath, (err, dossierContext) => {
            ...
            let dstPrefixPath = dstPath.substr(0, dossierContext.prefixPath.length);
            if (dstPrefixPath !== dossierContext.prefixPath) {
                return callback(Error('Destination is invalid. Renaming must be done in the scope of the same dossier'));
            }
            let relativeDstPath = dstPath.substr(dossierContext.prefixPath.length);
            relativeDstPath = pskPath.ensureIsAbsolute(relativeDstPath);
            dossierContext.archive.rename(dossierContext.relativePath, relativeDstPath, callback);
        });
    };
