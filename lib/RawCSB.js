...
function RawCSB() {
    ...
    this.readFile = (fileAlias, callback) => {
        bar.readFile(fileAlias, callback);
    };

    this.writeFile = (srcFilePath, fileAlias, callback) => {

        bar.addFile(srcFilePath, fileAlias, (err, barMapDigest) => {
            if (err) {
                return callback(err);
            }
            ...
            getBlockchain((err, bc)=>{
                ...
                bar.getFileHash(fileAlias, (err, fileDigest) => {
                    if (err) {
                        return callback(err);
                    }

                    const transaction = blockchain.startTransactionAs($$.securityContext.getCurrentAgentIdentity(), "StandardCSBTransactions", "addFileAnchor", fileAlias, fileDigest);
