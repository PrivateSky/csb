function EDFS(brickTransportStrategyName) {
    ...
    this.createCSB = () => {
        return new RawDossier(brickTransportStrategyName);
    };
...
//------------------------------------------------ internal methods -------------------------------------------------
    ...
    this.getRawDossier = (rawDossier, path, callback) => {
        rawDossier.listFiles((err, files) => {
            ...
            while (!barPath && splitPath.length > 0) {
                pathRest.unshift(splitPath.pop());
                barPath = files.find(barPath => barPath === splitPath.join("/"));
            }
            if (!barPath) {
            ...
            if (pathRest.length === 0) {
                return callback(undefined, {rawDossier, path: barPath});
            }

            getSeedFromManifest(rawDossier, barPath, pathRest.shift(), (err, seed) => {
                if (err) {
                    return callback(err);
                }

                const localRawDossier = self.loadBar(seed);
                this.getRawDossier(localRawDossier, pathRest.join("/"), callback);
            });
        });
    };

    function getSeedFromManifest(rawDossier, localPath, name, callback) {
        rawDossier.readFile(constants.MANIFEST_FILE, (err, data) => {
            if (err) {
                return callback(err);
            }

            const manifestContent = JSON.parse(data.toString());
            const mountPoint = manifestContent.mounts.find(mountPoint => mountPoint.localPath === localPath && mountPoint.name === name);
            callback(undefined, mountPoint.archiveIdentifier);
        });
    }
}
