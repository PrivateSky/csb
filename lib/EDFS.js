function EDFS(brickTransportStrategyName) {
    ...
    this.loadCSB = (seed, callback) => {
        console.log('loading csb with strategy', brickTransportStrategyName);
    ...
    this.createBarWithConstitution = function(folderConstitution, callback) {
        if(typeof folderConstitution === 'string') {
            folderConstitution = [];
        }

        const bar = this.createBar();

        bar.addAll(folderConstitution, constants.CSB.CONSTITUTION_FOLDER, (err, mapDigest) => {
            if (err) {
                return callback(err);
            }
